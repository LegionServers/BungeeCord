...
public class ServerConnector extends PacketHandler
{
    ...
    public void connected(ChannelWrapper channel) throws Exception
    {
        ...
        {
            String newHost = copiedHandshake.getHost() + "\00" + user.getEffectiveAddress().getHostString() + "\00" + user.getUUID();
            ...
            user.getCh().getHandle().eventLoop().schedule( new Runnable()
                {
                    ...
                    public void run()
                    {
                        if( user.isActive() )
                        {
                            user.connect( bungee.getServerInfo( user.getPendingConnection().getListener().getFallbackServer() ), null, true );
                        }
                    }
                }, 5, TimeUnit.SECONDS );
        }
    ...
    public void handle(Kick kick) throws Exception
    {
        ...
        ServerKickEvent origEvt = new ServerKickEvent( user, target, ComponentSerializer.parse( kick.getMessage() ), def, ServerKickEvent.State.CONNECTING );
